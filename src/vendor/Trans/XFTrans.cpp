...
XFTrans::XFTrans(QString appId, QString apiKey, QString apiSecret, QObject *parent) : 
    TransBase(parent), appId(appId), apiKey(apiKey), apiSecret(apiSecret), manager(this), endpoint(TRANS_ENDPOINT) {
    connect(&manager, &QNetworkAccessManager::finished, this, &XFTrans::onResult);
}

XFTrans::XFTrans(QString appId, QString apiKey, QString apiSecret, QString endpoint, QObject *parent) : 
    TransBase(parent), appId(appId), apiKey(apiKey), apiSecret(apiSecret), manager(this), endpoint(endpoint) {
    connect(&manager, &QNetworkAccessManager::finished, this, &XFTrans::onResult);
}
...
QString XFTrans::signBody(QByteArray body) {
    return QCryptographicHash::hash(body, QCryptographicHash::Sha256).toBase64();
}

QString XFTrans::genSignature(QString host, QString date, QString httpMethod, QString requestUri, QString digest) {
    ...
    return QMessageAuthenticationCode::hash(signatureStr.toLocal8Bit(), apiSecret.toLocal8Bit(), QCryptographicHash::Sha256).toBase64();
}
...
QNetworkRequest XFTrans::assembleRequest(QString url, QByteArray body) {
    ...
    QString dateStr = loc.toString(datetime, "ddd, dd MMM yyyy HH:mm:ss") + " GMT";
    req.setRawHeader("Date", dateStr.toLocal8Bit());
    QString digest = "SHA-256=" + signBody(body);
    req.setRawHeader("Digest", digest.toLocal8Bit());
    QString sign = genSignature(qurl.host(), dateStr, "POST", qurl.path(), digest);
    QString authHeader = "api_key=\"%1\", algorithm=\"hmac-sha256\", headers=\"host date request-line digest\", signature=\"%2\"";
